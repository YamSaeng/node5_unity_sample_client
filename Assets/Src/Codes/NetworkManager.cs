...
public class NetworkManager : MonoBehaviour
{
    ...
    {
        ProcessPingPacketResponseData(packetData);
    }

    public async void SendPongPacket(long serverTimestamp)
    {        
        ...
        var payloadWriter = new ArrayBufferWriter<byte>();
        Packets.Serialize(payloadWriter, C2S_PongPacket);
        byte[] payloadData = payloadWriter.WrittenSpan.ToArray();
    ...
    void ProcessPingPacketResponseData(byte[] data)
    {
        try
        {
            Ping pingPacket = Packets.Deserialize<Ping>(data);
            SendPongPacket(pingPacket.timestamp);
        }
        catch (Exception error)
        {
            Debug.LogError($"Error processing response data: {error.Message}");
        }
    }
    ...
    void HandleNormalPacket(byte[] packetData)
    {
        ...
        if (response.data != null && response.data.Length > 0)
        {
            ...
            ProcessNormalPacketResponseData(response.data);
        }
    }

    void ProcessNormalPacketResponseData(byte[] data)
    {
        try
        {
            // Init Data를 받을때 x, y 좌표를 추가로 받아서 위치값을 변경
            InitialPayload response = Packets.Deserialize<InitialPayload>(data);
            GameManager.instance.player.gameObject.transform.position =
                new Vector3((float)response.x, (float)response.y, 0);
        }
        catch (Exception e)
        {
